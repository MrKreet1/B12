...
#### 5.5.1. Наиболее низкоэнергетическая структура в исследованной выборке
...
Следовательно, именно эта структура должна рассматриваться как наиболее низкоэнергетическая форма в пределах исследованного набора стартовых геометрий, мультиплетностей и использованного уровня теории.
...
Таким образом, кольцевой мотив не является вычислительным артефактом, но заметно проигрывает наиболее низкоэнергетической структуре исследованной выборки.
...
Для всех финальных структур разность между `dft_energy_hartree` и `numfreq_energy_hartree` остается очень малой. Максимальное абсолютное расхождение составляет `2.16 × 10^-8 Eh`, что соответствует примерно `5.67 × 10^-5 кДж/моль`. Это подтверждает внутреннюю согласованность итоговых расчетов и показывает, что частотный анализ выполнялся на корректно определенных финальных геометриях.

### 5.6. Графическая иллюстрация результатов

Для дипломной интерпретации наиболее наглядными являются графики, непосредственно показывающие сужение набора структур, энергетическое ранжирование финальных кандидатов, частотную устойчивость и влияние стартового приближения на результат расчета.

**Рисунок 1.** Воронка расчетного конвейера.

![Воронка расчетного конвейера](../seaborn_charts/out/07_diploma_funnel.png)

На воронке показана ключевая логика работы конвейера: `104` рабочих xTB-задач приводят только к `16` сошедшимся xTB-минимумам, после удаления дублей остаются `3` уникальных кандидата, а строгую частотную проверку проходят лишь `2` структуры. Этот рисунок является одним из главных аргументов в пользу многостартового подхода и автоматической фильтрации.

**Рисунок 2.** Распределение относительных xTB-энергий для всех сошедшихся расчетов.

![Распределение xTB-энергий](../seaborn_charts/out/08_xtb_energy_distribution.png)

График показывает широкий энергетический разброс уже на стадии дешевого скрининга. Следовательно, выбор начальной геометрии оказывает существенное влияние на результат, а многостартовый поиск не является формальностью.

**Рисунок 3.** Относительные энергии финальных DFT-структур.

![Относительные энергии финальных DFT-структур](../seaborn_charts/out/09_final_dft_relative_energy.png)

На рисунке ясно видно, что структура `double_ring_6_6_d1.90_m1_r2scan` является безусловным лидером по энергии в исследованной выборке. Кольцевая структура `ring12_d1.70_m1_r2scan` сохраняет устойчивость, но расположена намного выше, а `zigzag_chain12_d1.50_m3_r2scan` оказывается и менее выгодной по энергии, и не проходит частотную верификацию.

**Рисунок 4.** Минимальная частота для финальных кандидатов.

![Минимальная частота финальных кандидатов](../seaborn_charts/out/10_final_min_frequency.png)

Этот график особенно важен для защиты, поскольку позволяет сразу отделить устойчивые структуры от неустойчивой. У двух принятых минимумов минимальная частота близка к нулю и статус равен `OK_NO_IMAG`, тогда как для `zigzag_chain12_d1.50_m3_r2scan` минимальная частота составляет `-2022.99 см⁻¹`, что свидетельствует о выраженной колебательной неустойчивости.

**Рисунок 5.** Тепловая карта «тип старта × стартовое расстояние × успех xTB-сходимости».

![Тепловая карта стартовых условий](../seaborn_charts/out/11_start_distance_heatmap.png)

Тепловая карта показывает, какие стартовые мотивы и расстояния чаще приводили к сошедшимся xTB-минимумам. Это полезно для интерпретации качества стартовых приближений и для планирования дальнейшего расширения набора начальных структур.

**Рисунок 6.** Три финальные геометрии B12 после `r2SCAN-3c`-оптимизации.

![Три финальные геометрии B12](../seaborn_charts/out/12_final_structure_triptych.png)

Совместный рисунок трех финальных геометрий удобен для визуального сопоставления структурных мотивов. Под каждой геометрией указаны мультиплетность, относительная энергия и итоговый частотный статус, что делает рисунок особенно полезным для дипломной защиты.
...
Итоговый вывод состоит в том, что в пределах использованного набора стартовых геометрий, рассмотренных мультиплетностей и уровня теории `r2SCAN-3c` наиболее низкоэнергетической структурой кластера B12 в исследованной выборке является двухкольцевой синглетный мотив `double_ring_6_6`.
